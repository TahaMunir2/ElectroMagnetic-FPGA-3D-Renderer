...
//  contour_hdmi_top.sv
//  ----------------------------------------------------------------------------
//  Standalone PYNQ-Z1 HDMI top for the 2D contour renderer. This is a faithful
//  copy of ray_unit_hdmi_top_d4s48 with three changes:
//    1. ray_unit4 (+ its 48 marcher / 2 normal BRAMs) replaced by ONE
//       contour_unit reading a SINGLE heightmap_bram.
//    2. All camera / sun / lighting constants removed.
//    3. VIDEO_DELAY_PIX resized from the contour unit's real latency
//       (LAT = RD_LAT + 2 core cycles) instead of the ray unit's 542.
//
//  Everything else (clk_wiz_1, dual-domain reset sync, the 800x525 core-domain
//  timing generator running 1 active pixel / 4 core cycles, the async FIFO,
//  video_timing_640x480, magenta underflow diagnostic, rgb2dvi_0) is unchanged,
//  so this drops onto the same board design and constraints.
//
//  Required Vivado IP (same as before):
//    clk_wiz_1: 125 MHz in -> 25 MHz pix, 125 MHz serial, 100 MHz core
...
module contour_hdmi_top (
    ...
    // ---- contour unit latency -> video delay --------------------------------
    // Set RD_LAT to your heightmap_bram's true read latency (1 or 2).
    localparam int RD_LAT              = 1;
    localparam int RENDER_LATENCY_CORE = RD_LAT + 2;                 // = LAT
    ...
    // ---- dual-domain reset synchronisation ----------------------------------
    ...
    contour_unit #(
        .W(W), .H(H), .GRID_N(GRID_N), .H_W(H_W), .H_I(2), .RD_LAT(RD_LAT)
    ) u_contour (
    ...
    always_ff @(posedge clk_pix) begin
        if (!rst_pix_n) begin
            ...
            hdmi_de    <= delayed_active; // never gate VDE with FIFO status
            ...
            if (fifo_rd_en) begin
                ...
                hdmi_r <= 8'hff; hdmi_g <= 8'h00; hdmi_b <= 8'hff; // magenta diag
            end else begin
                hdmi_r <= 8'd0; hdmi_g <= 8'd0; hdmi_b <= 8'd0;
            end
        end
