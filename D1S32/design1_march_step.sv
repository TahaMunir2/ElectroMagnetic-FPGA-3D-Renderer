// ============================================================================
//  march_step.sv  (rewritten â€?clean, readable, no helper functions)
//  ----------------------------------------------------------------------------
...
module design1_march_step #(
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (status_in == ST_MARCHING) begin
                ...
                // Frozen ray â€?position doesn't change
                Px_A <= Px_in;
                Py_A <= Py_in;
                Pz_A <= Pz_in;
            end
    ...
    // Raw grid coordinates (POS_W+1 bits signed â€?extra bit catches overflow)
