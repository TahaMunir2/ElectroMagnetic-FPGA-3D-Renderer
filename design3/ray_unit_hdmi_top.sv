...
//   march_step3 captures BRAM data one cycle after issuing the address and
//   assumes the data is valid regardless of the `re` line state (standard
//   Xilinx RAMB18 behaviour: the output register always clocks on clk-enable).
//   The heightmap_bram model below MUST therefore drive `dout` from the
//   registered address every cycle and must NOT gate the data path on `re`.
//   If `re` gates the data, the left-column corners will not land and hits
...
module ray_unit_hdmi_top (
    ...
    // Heightmap BRAMs — 32 marcher (16 copies) + 2 normal (1 copy),
    // clocked on clk_core.
