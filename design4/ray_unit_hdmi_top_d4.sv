...
//   - The renderer core (ray_unit4) produces 1 pixel every 4 core cycles.
//     To sustain 25 Mpix/s scanout the core must run at 4x the pixel clock
//     => clk_core = 100 MHz (NOTE: faster than Design 3's 50 MHz core).
//   - An asynchronous FIFO crosses from the 100 MHz render domain to the
...
//
// What changed vs Design 3:
//   - Core swapped ray_unit3 -> ray_unit4.  SAME bilinear algorithm (smooth
//     silhouettes); the 4 corner reads are folded across 4 cycles on a SINGLE
//     port per step instead of 2 cycles on 2 ports.  Quarter-rate throughput.
//   - Marcher BRAM port shape: 2-D [N_STEPS][2] (32 ports, 16 copies) ->
//     flat [N_STEPS] (16 ports, 8 copies).
//   - Heightmap copies: marcher 16 -> 8 (+ normal 1) => 9 copies = 18 BRAM18.
//   - Normal ports unchanged (still 2 ports / 1 copy).
//   - clk_core bumped 50 MHz -> 100 MHz to keep 25 Mpix/s at quarter rate.
//     (If 100 MHz timing is tight, drop scanout to a lower mode or accept
//      sub-60fps; the half-/quarter-rate maths is documented in ray_unit4.)
//
// IMPORTANT BRAM CONTRACT (same as Design 3):
//   march_step4 captures BRAM data one cycle after issuing the address and
//   assumes the data is valid regardless of the `re` line state (standard
//   Xilinx RAMB18 behaviour: output register always clocks on clk-enable).
//   The heightmap_bram model MUST drive `dout` from the registered address
//   every cycle and must NOT gate the data path on `re`.
//
// Required Vivado IP:
//   - clk_wiz_0:
