...
% ---------------------------------------------------------------------
\subsection{Keeping the Solver and the Display Out of Each Other's Way}

This is the part of the integration we are most pleased with, because it solves
a genuine timing conflict cleanly. The solver finishes a new frame whenever it
happens to --- a rate that depends on the grid size and how much we have slowed
it for viewing --- while the renderer has no such freedom: it must read a frame
every twenty-fifth of a second to feed the display. If we simply pointed the
...
% ---------------------------------------------------------------------
\subsection{Going Faster: Multi-Lane Parallelism}
...
identical result. The price is paid in memory rather than logic: each lane needs
its own memories and read ports, and neighbouring lanes have to swap a single
boundary row, because the update at the edge of one lane reaches into the next.
Interestingly the four-lane version actually used \emph{fewer} block RAMs than
the single-lane one (108 against 114), simply because four smaller memories pack
into the device's RAM blocks more tightly than one large one. We did try eight
and sixteen lanes but abandoned them: beyond four the extra speed stops being
worth the growing boundary-exchange logic and routing congestion, especially as
the solver was by then already running far faster than the display could show.

% ---------------------------------------------------------------------
\subsection{The Two Demonstrations}

The same solver drives two different renderers, and the choice between them is
really a trade between block-RAM use and clock speed (Table~\ref{tab:demos}).
The first is a flat top-down contour heatmap (Fig.~\ref{fig:2d}). It needs only a
single heightmap memory, and freeing up the rest of the block RAM is exactly
what let us quadruple the grid to $128\times128$ for finer detail; with the
...
three-dimensional landscape by ray-marching (Fig.~\ref{fig:3d}). This is the more
striking view but a far hungrier one --- it needs around fifty copies of the
heightmap --- so it runs at 100\,MHz and folds its memory reads across several
cycles to fit, and its camera can be orbited live from the notebook. Keeping
both is deliberate: the 2D view is the clearer one for reading the physics, and
the 3D view is the one that makes the demonstration memorable.
