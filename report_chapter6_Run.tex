...
%  More diagram, less word version
%
...
\midrule
Overall cover (W$\times$D$\times$H) & $179 \times 178 \times 67$\,mm \\
Walls / top plate & 3\,mm / 5\,mm \\
Touch-panel window & hand-shaped recess, on the back edge of lid for setting input wire \\
FPGA bay & $127 \times 92$\,mm open cut-out, back-left; only HDMI, USB and power supply remain exposed, while the board buttons are covered \\
Control strip & open breadboard strip exposing all controls directly: 7 knobs and 4 switches \\
Cable slot & $\sim$30 $\times$ 20\,mm, on back wall, for input, ground, probe wires of Mode 1 \\
...
\begin{figure}[ht]
\centering
\figplaceholder{the dimensioned top-view layout of the cover, marking the panel window, the FPGA bay, the open breadboard control strip and the rear cable slot}
...
Breadboards mount to the baseboard, the framed panel fits the top face, the FPGA board sits in its bay with only the required connectors exposed, and the breadboard control strip remains directly accessible through its opening. Being a simple cover it has no internal standoffs and a single panel window --- exactly what the final one-panel design needs.
