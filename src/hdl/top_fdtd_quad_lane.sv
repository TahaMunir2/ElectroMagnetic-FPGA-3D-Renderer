...
module top_fdtd_quad_lane #(
    ...
    assign ey_rd_addr_0[0] = slv_ey_rd_addr[0];
    assign ey_rd_addr_0[1] = slv_ey_rd_addr[1];
    assign ey_rd_addr_0[2] = slv_ey_rd_addr[2];
    assign ey_rd_addr_0[3] = slv_ey_rd_addr[3];

    assign ex_rd_addr_0[0] = slv_ex_rd_addr[0];
    assign ex_rd_addr_0[1] = slv_ex_rd_addr[1];
    assign ex_rd_addr_0[2] = slv_ex_rd_addr[2];
    assign ex_rd_addr_0[3] = slv_ex_rd_addr[3];

    assign bz_rd_addr_0[0] = slv_bz_rd_addr[0];
    assign bz_rd_addr_0[1] = slv_bz_rd_addr[1];
    assign bz_rd_addr_0[2] = slv_bz_rd_addr[2];
    assign bz_rd_addr_0[3] = slv_bz_rd_addr[3];

    assign ey_we[0] = slv_ey_we[0];
    assign ey_wr_addr[0] = slv_ey_wr_addr[0];
    assign ey_wr_data[0] = slv_ey_wr_data[0];
    assign ey_we[1] = slv_ey_we[1];
    assign ey_wr_addr[1] = slv_ey_wr_addr[1];
    assign ey_wr_data[1] = slv_ey_wr_data[1];
    assign ey_we[2] = slv_ey_we[2];
    assign ey_wr_addr[2] = slv_ey_wr_addr[2];
    assign ey_wr_data[2] = slv_ey_wr_data[2];
    assign ey_we[3] = slv_ey_we[3];
    assign ey_wr_addr[3] = slv_ey_wr_addr[3];
    assign ey_wr_data[3] = slv_ey_wr_data[3];

    assign ex_we[0] = slv_ex_we[0];
    assign ex_wr_addr[0] = slv_ex_wr_addr[0];
    assign ex_wr_data[0] = slv_ex_wr_data[0];
    assign ex_we[1] = slv_ex_we[1];
    assign ex_wr_addr[1] = slv_ex_wr_addr[1];
    assign ex_wr_data[1] = slv_ex_wr_data[1];
    assign ex_we[2] = slv_ex_we[2];
    assign ex_wr_addr[2] = slv_ex_wr_addr[2];
    assign ex_wr_data[2] = slv_ex_wr_data[2];
    assign ex_we[3] = slv_ex_we[3];
    assign ex_wr_addr[3] = slv_ex_wr_addr[3];
    assign ex_wr_data[3] = slv_ex_wr_data[3];

    assign bz_we[0] = slv_bz_we[0];
    assign bz_wr_addr[0] = slv_bz_wr_addr[0];
    assign bz_wr_data[0] = slv_bz_wr_data[0];
    assign bz_we[1] = slv_bz_we[1];
    assign bz_wr_addr[1] = slv_bz_wr_addr[1];
    assign bz_wr_data[1] = slv_bz_wr_data[1];
    assign bz_we[2] = slv_bz_we[2];
    assign bz_wr_addr[2] = slv_bz_wr_addr[2];
    assign bz_wr_data[2] = slv_bz_wr_data[2];
    assign bz_we[3] = slv_bz_we[3];
    assign bz_wr_addr[3] = slv_bz_wr_addr[3];
    assign bz_wr_data[3] = slv_bz_wr_data[3];
