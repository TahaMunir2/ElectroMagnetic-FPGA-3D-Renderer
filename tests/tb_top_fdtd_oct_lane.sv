...
module tb_top_fdtd_oct_lane;

    localparam LANES      = 8;
    localparam TOTAL_ROWS = 128;
    localparam ROWS       = 16;
    localparam COLUMNS    = 128;
    localparam GRID_SIZE  = ROWS * COLUMNS;   // 2048 per lane
    ...
    // Arm one more iteration (negedge-safe toggling)
    ...
    initial begin
        ...
        source_in    = 16'sd8192;
        source_valid = 1'b1;
        source_addr  = 8 * COLUMNS + 8;   // global (8,8) → lane 0 local 8*128+8=1032
        solver_enable = 1'b1;
        ...
        // ─────────────────────────────────────────────────────
        // TEST 2: Source injection — Ey at lane 0 local (8,8) must be nonzero
        // ─────────────────────────────────────────────────────
        ...
        $display("  Ey[8][8] = %0d  PASS",
                 $signed(dut.bram_0.ey_mem_0[lflat(8, 8)]));
        ...
        // ─────────────────────────────────────────────────────
        // TEST 4: Ey bottom boundary zero — global row 127 (lane 7 local row 15)
        // ─────────────────────────────────────────────────────
        $display("TEST 4: Ey bottom boundary (global row 127) forced zero");
        for (col = 0; col < COLUMNS; col++) begin
            if (dut.bram_7.ey_mem_0[lflat(ROWS-1, col)] !== '0) begin
                $display("  FAIL: Ey[127][%0d] = %0d", col,
                         $signed(dut.bram_7.ey_mem_0[lflat(ROWS-1, col)]));
                $finish;
            end
        end
        $display("  PASS");
        ...
        // Source at global (8,8). Run 20 more iterations.
        // Wave diffuses from lane 0 into lane 1 (global rows 16-31).
        // A broken halo leaves lane 1 all-zero indefinitely.
        // ─────────────────────────────────────────────────────
        ...
        if (!nonzero_found) begin
            $display("  FAIL: lane 1 Ey all-zero — halo likely broken");
            $finish;
        end
