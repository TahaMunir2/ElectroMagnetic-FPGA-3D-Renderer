...
// ============================================================================
//  march_step32.sv  =  march_step4  +  hit-cell corner forwarding.
//  ----------------------------------------------------------------------------
//  IDENTICAL to march_step4 (instantiate with GRID_N=128 -> IDX_W=7, and
//  STEP_W=6 for up to 32 steps) EXCEPT it forwards the three heightmap corners
//  of the HIT cell -- h00=h(ix,iy), h10=h(ix+1,iy), h01=h(ix,iy+1) -- down the
//  chain so the downstream `normal` can build a forward-difference normal
//  WITHOUT any BRAM reads of its own.
//
//  Two new carries, both mechanical:
//   * cell corners  (h00c/h10c/h01c): captured at stage D from the live B4
//     corner registers (same source the lerp uses), carried D->D2, consumed at
//     D3 when a hit is registered.
//   * hit corners   (h00h/h10h/h01h): persistent, propagate exactly like h_hit
//     -- updated at D3 on a hit, otherwise passed through every stage.
//  Bit-exact with march_step4 on all existing outputs; the corners are purely
//  additive.
// ============================================================================
module march_step32 #(
    ...
    // hit-cell corners, chained from the previous step (forward-diff normal)
    input  logic signed [H_W-1:0]      h00h_in,
    input  logic signed [H_W-1:0]      h10h_in,
    input  logic signed [H_W-1:0]      h01h_in,
    ...
    output logic signed [H_W-1:0]      h00h_out,
    output logic signed [H_W-1:0]      h10h_out,
    output logic signed [H_W-1:0]      h01h_out,
    ...
    // ----- corner-forwarding pipeline registers (additive) -----
    // hit corners: persistent, mirror h_hit through every stage
    logic signed [H_W-1:0] h00h_A,  h10h_A,  h01h_A;
    logic signed [H_W-1:0] h00h_B0, h10h_B0, h01h_B0;
    logic signed [H_W-1:0] h00h_B1, h10h_B1, h01h_B1;
    logic signed [H_W-1:0] h00h_B2, h10h_B2, h01h_B2;
    logic signed [H_W-1:0] h00h_B3, h10h_B3, h01h_B3;
    logic signed [H_W-1:0] h00h_B4, h10h_B4, h01h_B4;
    logic signed [H_W-1:0] h00h_B5, h10h_B5, h01h_B5;
    logic signed [H_W-1:0] h00h_D,  h10h_D,  h01h_D;
    logic signed [H_W-1:0] h00h_D2, h10h_D2, h01h_D2;
    logic signed [H_W-1:0] h00h_D3, h10h_D3, h01h_D3;
    // cell corners of the CURRENT cell, captured at D (from B4), used at D3
    logic signed [H_W-1:0] h00c_D,  h10c_D,  h01c_D;
    logic signed [H_W-1:0] h00c_D2, h10c_D2, h01c_D2;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            v_A          <= valid_in;
            h00h_A <= h00h_in; h10h_A <= h10h_in; h01h_A <= h01h_in;
        end
    end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            h_hit_B0 <= h_hit_A;
            h00h_B0 <= h00h_A; h10h_B0 <= h10h_A; h01h_B0 <= h01h_A;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) v_B1 <= 1'b0;
            ...
            h00h_B1<=h00h_B0; h10h_B1<=h10h_B0; h01h_B1<=h01h_B0;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) v_B2 <= 1'b0;
            ...
            h00h_B2<=h00h_B1; h10h_B2<=h10h_B1; h01h_B2<=h01h_B1;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) v_B3 <= 1'b0;
            ...
            h00h_B3<=h00h_B2; h10h_B3<=h10h_B2; h01h_B3<=h01h_B2;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) v_B4 <= 1'b0;
            ...
            h00h_B4<=h00h_B3; h10h_B4<=h10h_B3; h01h_B4<=h01h_B3;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) v_B5 <= 1'b0;
            ...
            h00h_B5<=h00h_B4; h10h_B5<=h10h_B4; h01h_B5<=h01h_B4;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) v_D <= 1'b0;
            ...
            h_top_D <= h_top_comb;
            h_bot_D <= h_bot_comb;
            h00h_D<=h00h_B5; h10h_D<=h10h_B5; h01h_D<=h01h_B5;
            // current-cell corners: same B4 source the lerp reads this cycle
            h00c_D<=h00_B4; h10c_D<=h10_B4; h01c_D<=h01_B4;
        end
    end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) v_D2 <= 1'b0;
            ...
            h00h_D2<=h00h_D; h10h_D2<=h10h_D; h01h_D2<=h01h_D;
            h00c_D2<=h00c_D; h10c_D2<=h10c_D; h01c_D2<=h01c_D;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) v_D3 <= 1'b0;
            ...
            if (stat_D2 == ST_MARCHING && crossed_D3) begin
                ...
                Px_hit_D3 <= Px_hit_D2;
                Py_hit_D3 <= Py_hit_D2;
                h00h_D3 <= h00c_D2; h10h_D3 <= h10c_D2; h01h_D3 <= h01c_D2;
            end else begin
                stat_D3   <= stat_D2;
                h_hit_D3  <= h_hit_D2;
                h00h_D3 <= h00h_D2; h10h_D3 <= h10h_D2; h01h_D3 <= h01h_D2;
                ix_hit_D3 <= ix_hit_D2;
                iy_hit_D3 <= iy_hit_D2;
                Px_hit_D3 <= Px_hit_D2;
                Py_hit_D3 <= Py_hit_D2;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) valid_out <= 1'b0;
            ...
            h00h_out<=h00h_D3; h10h_out<=h10h_D3; h01h_out<=h01h_D3;
