...
set proj_name "MVP2_fdtd_contour_128_cb"
...
# Renderer sources (2D contour: module-ref contour_renderer_core_bd + 1 heightmap)
set rend_files [list \
    [file join $rtl_dir renderer contour_renderer_core_bd.v]      \
    [file join $rtl_dir renderer contour_unit_cb.sv]              \
...
# ---- axi_gpio_palette (M03) — PS-writable 16-entry contour palette ----
#   CH1 out: pal_word = {idx[27:24], rgb[23:0]} (one palette entry)
#   CH2 out: bit0 = commit toggle (renderer pulses pal_we on each edge)
create_bd_cell -type ip -vlnv xilinx.com:ip:axi_gpio:2.0 axi_gpio_palette
set_property -dict [list CONFIG.C_GPIO_WIDTH {32} CONFIG.C_GPIO2_WIDTH {32} \
    CONFIG.C_ALL_OUTPUTS {1} CONFIG.C_ALL_OUTPUTS_2 {1} CONFIG.C_IS_DUAL {1}] [get_bd_cells axi_gpio_palette]
connect_bd_net $FCLK     [get_bd_pins axi_gpio_palette/s_axi_aclk]
connect_bd_net $PS_ARSTN [get_bd_pins axi_gpio_palette/s_axi_aresetn]
connect_bd_intf_net [get_bd_intf_pins axi_ic_0/M03_AXI] [get_bd_intf_pins axi_gpio_palette/S_AXI]
make_slice slice_pal_word    27 0  32
make_slice slice_pal_commit   0 0  32
connect_bd_net [get_bd_pins axi_gpio_palette/gpio_io_o]  [get_bd_pins slice_pal_word/Din]
connect_bd_net [get_bd_pins axi_gpio_palette/gpio2_io_o] [get_bd_pins slice_pal_commit/Din]
connect_bd_net [get_bd_pins slice_pal_word/Dout]   [get_bd_pins renderer_0/pal_word]
connect_bd_net [get_bd_pins slice_pal_commit/Dout] [get_bd_pins renderer_0/pal_commit]
...
catch { set_property offset 0x41230000 [get_bd_addr_segs {axi_gpio_status/S_AXI/Reg}] }
catch { set_property offset 0x41210000 [get_bd_addr_segs {axi_gpio_motion/S_AXI/Reg}] }
catch { set_property offset 0x41220000 [get_bd_addr_segs {axi_gpio_palette/S_AXI/Reg}] }
