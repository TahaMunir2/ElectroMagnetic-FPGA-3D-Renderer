...
module s_mag_to_heightmap_bridge #(
    parameter int ADDR_W       = 14,   // s_mag side: 128x128 = 16384 cells
    parameter int HM_ADDR_W    = 12,   // heightmap side: 64x64 = 4096 cells (2x downsample)
    parameter int DST_N        = 64,   // heightmap grid edge
    parameter int SRC_N        = 128,  // s_mag grid edge
    parameter int DATA_W       = 16,
    parameter int GUARD_CYCLES = 384,  // wait for D4S80DO render pipeline to drain
    ...
    // Heightmap write side — broadcast to all heightmap copies (64x64)
    output logic                     hm_we,
    output logic [HM_ADDR_W-1:0]     hm_waddr,
    output logic signed [DATA_W-1:0] hm_wdata,

    // Status
    output logic                     busy
);
    localparam int DEPTH    = 1 << HM_ADDR_W;        // 4096 destination cells (64x64)
    localparam [HM_ADDR_W-1:0] LAST = DEPTH - 1;
    ...
    logic [HM_ADDR_W-1:0]     rd_addr;      // DESTINATION (heightmap) index being issued
    ...
    // 2x downsample: destination (dr,dc) reads s_mag at (dr*2, dc*2). For
    // DST_N=64, SRC_N=128: src = dr*256 + dc*2 = {dr[5:0],1'b0,dc[5:0],1'b0}.
    localparam int HALF = HM_ADDR_W/2;   // bits per dst axis (6 for 64)
    always_comb begin
        s_mag_addrb = {rd_addr[HM_ADDR_W-1:HALF], 1'b0, rd_addr[HALF-1:0], 1'b0};
