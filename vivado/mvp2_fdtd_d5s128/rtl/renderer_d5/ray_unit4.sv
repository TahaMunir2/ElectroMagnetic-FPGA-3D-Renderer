// ============================================================================
//  ray_unit4.sv  (D5S128 top-level single-pixel renderer)
//  ----------------------------------------------------------------------------
//  Chains ray_gen -> marcher4 -> normal4 -> shader.
//
//  Design 4: bilinear interpolation INSIDE the marcher (smooth silhouettes,
//  same algorithm as Design 3), but the 4 corner reads are folded across 4
//  cycles on a SINGLE port per step.  Quarter-rate (1 pixel / 4 cycles).
//
//  BRAM ports exposed:
//      marcher4 : N_STEPS * 1 = 128 logical read ports (flat unpacked array [N_STEPS])
//      normal4  : 2 ports
//      D5 pairs those read ports into 64 dual-read marcher BRAMs + 1 normal BRAM.
//
//  Pipeline latency (D5S128):
//      ray_gen  :   4 cycles
//      marcher4 : 1536 cycles (128 march_step4 * 12-cycle latency)
//      normal4  :   5 cycles
//      shader   :   5 cycles
//      TOTAL    : 1550 cycles
//
...
module ray_unit4 #(
    ...
    // ----- Marcher BRAM ports: 1 per step, NOT shared (flat [N_STEPS]) -----
    ...
    // ----- Normal BRAM ports (2 ports / 1 copy) -----
