`timescale 1ns/1ps
// ============================================================================
//  d5s128h_core_impl.sv
//  D5S128H 3D ray-march renderer as a BD core for the FDTD pipeline. This is
...
//    - the read-only heightmap_bram is replaced by paired dual-read D5 copies.
//      Bridge writes enter via a small async FIFO and are broadcast to all copies
//      in the clk_core domain.
...
//  N_STEPS=128, GRID_N=64, half-cell march step.
// ============================================================================
module d5s128h_core_impl (
    input  wire        clk_pix,
    input  wire        clk_core,
    ...
    localparam int N_STEPS = 128, H_W = 16, DIR_W = 16, POS_W = 16;
    ...
    // ---- D5 shared heightmap copies ----
    // Bridge writes arrive in clk_pix, then are consumed at clk_core. Each D5
    // heightmap copy is true-dual-port in clk_core: port A is write-or-read,
    // port B is read-only. During active rendering, paired march steps use both
    // read ports. During the vblank update burst, the bridge guard has already
    // drained the renderer, so port A can safely broadcast writes.
    localparam int N_MARCH_BRAMS = (N_STEPS + 1) / 2;
    localparam int HM_WR_W = ADDR_W + H_W;
    ...
    logic                  hm_wr_full, hm_wr_empty;
    logic                  hm_wr_rd_rst_busy, hm_wr_wr_rst_busy;
    logic [HM_WR_W-1:0]    hm_wr_fifo_dout;
    wire                   hm_wr_fifo_wr_en = hm_we & ~hm_wr_full & ~hm_wr_wr_rst_busy;
    wire                   hm_wr_core_valid = ~hm_wr_empty & ~hm_wr_rd_rst_busy;
    wire [ADDR_W-1:0]      hm_wr_addr_core = hm_wr_fifo_dout[HM_WR_W-1 -: ADDR_W];
    wire signed [H_W-1:0]  hm_wr_data_core = hm_wr_fifo_dout[H_W-1:0];

    xpm_fifo_async #(
        .FIFO_MEMORY_TYPE("auto"), .FIFO_WRITE_DEPTH(1024),
        .WRITE_DATA_WIDTH(HM_WR_W), .READ_DATA_WIDTH(HM_WR_W), .READ_MODE("fwft"),
        .FIFO_READ_LATENCY(0), .PROG_FULL_THRESH(768),
        .USE_ADV_FEATURES("0000"), .CDC_SYNC_STAGES(2), .RELATED_CLOCKS(0)
    ) u_heightmap_write_fifo (
        .wr_clk(clk_pix), .rst(~rst_pix_n), .wr_en(hm_wr_fifo_wr_en),
        .din({hm_waddr, hm_wdata}), .full(hm_wr_full), .wr_rst_busy(hm_wr_wr_rst_busy),
        .rd_clk(clk_core), .rd_en(hm_wr_core_valid), .dout(hm_wr_fifo_dout),
        .empty(hm_wr_empty), .rd_rst_busy(hm_wr_rd_rst_busy),
        .sleep(1'b0), .injectsbiterr(1'b0), .injectdbiterr(1'b0),
        .almost_empty(), .almost_full(), .data_valid(), .dbiterr(),
        .overflow(), .prog_empty(), .prog_full(), .rd_data_count(), .wr_data_count(),
        .sbiterr(), .underflow(), .wr_ack()
    );

    genvar gi;
    generate
        for (gi = 0; gi < N_MARCH_BRAMS; gi++) begin : g_marcher_bram
            heightmap_bram_d5_dualread #(.ADDR_W(ADDR_W), .DATA_W(H_W)) u_bram (
                .clk(clk_core),
                .we(hm_wr_core_valid), .waddr(hm_wr_addr_core), .wdata(hm_wr_data_core),
                .addr_a(mb_addr[2*gi]), .re_a(mb_re[2*gi]), .dout_a(mb_dout[2*gi]),
                .addr_b(mb_addr[2*gi+1]), .re_b(mb_re[2*gi+1]), .dout_b(mb_dout[2*gi+1]));
        end
        heightmap_bram_d5_dualread #(.ADDR_W(ADDR_W), .DATA_W(H_W)) u_normal_bram (
            .clk(clk_core),
            .we(hm_wr_core_valid), .waddr(hm_wr_addr_core), .wdata(hm_wr_data_core),
            .addr_a(nb_addr[0]), .re_a(nb_re[0]), .dout_a(nb_dout[0]),
            .addr_b(nb_addr[1]), .re_b(nb_re[1]), .dout_b(nb_dout[1]));
    endgenerate
