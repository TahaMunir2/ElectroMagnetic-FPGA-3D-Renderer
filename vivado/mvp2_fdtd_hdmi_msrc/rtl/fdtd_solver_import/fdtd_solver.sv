...
module fdtd_solver #(
    ...
    input  wire [2*CELL_WIDTH-1:0]  source_addr,    // source 0 (kept for compat)
    // Additional coherent sources for interference. All driven by the same
    // CORDIC sample (latched once per iteration), each at its own cell.
    input  wire [2*CELL_WIDTH-1:0]  source_addr1,
    input  wire [2*CELL_WIDTH-1:0]  source_addr2,
    input  wire [2*CELL_WIDTH-1:0]  source_addr3,
    input  wire [3:0]               source_en,      // per-source enable {3,2,1,0}
...
// Coherent multi-source: one CORDIC sample latched per iteration (src_sample),
// injected at every enabled source cell -> stable interference fringes.
logic signed [DATA_WIDTH-1:0] src_sample;
wire src_hit = source_valid & (
      (source_en[0] & (wr_cell == source_addr )) |
      (source_en[1] & (wr_cell == source_addr1)) |
      (source_en[2] & (wr_cell == source_addr2)) |
      (source_en[3] & (wr_cell == source_addr3)) );
...
always_ff @(posedge clk) begin
    ...
    if (rst || !solver_enable) begin
        ...
        // latch one coherent source sample at the start of each Ey pass
        if (counter == 0) src_sample <= source_in;

        if (counter < GRID_SIZE) begin
            ey_we <= write_valid;
            ex_we <= write_valid;
            if (wr_row == 0 || wr_row == CELLS-1) ey_wr_data <= '0;
            else if (src_hit) ey_wr_data <= engine_ey_new + src_sample; // soft source(s): add coherent drive (no hard reflector)
