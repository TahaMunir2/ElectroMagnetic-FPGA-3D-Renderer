...
set proj_name "MVP2_fdtd_hdmi_msrc"
set proj_dir  "E:/Vivado/Projects/desperate_yi/MVP2_fdtd_hdmi_msrc"
...
package_ip $part $ip_repo $ip_work \
    ...
    "MVP2 field BRAM 16x4096 true-dual-port."
...
# ---------------------------------------------------------------------------
#  FDTD solver adapter (clk_pix)
# ---------------------------------------------------------------------------
create_bd_cell -type ip -vlnv user.org:user:fdtd_solver_bd_adapter:1.0 fdtd_solver_bd_adapter_0
connect_bd_net $CLKP    [get_bd_pins fdtd_solver_bd_adapter_0/clk]
connect_bd_net $PIX_RST [get_bd_pins fdtd_solver_bd_adapter_0/rst]
connect_bd_net [get_bd_pins cordic_source_adapter_0/source_q313]  [get_bd_pins fdtd_solver_bd_adapter_0/source_q313]
connect_bd_net [get_bd_pins cordic_source_adapter_0/source_valid] [get_bd_pins fdtd_solver_bd_adapter_0/source_valid]

# ---------------------------------------------------------------------------
#  Field magnitude (clk_pix)
# ---------------------------------------------------------------------------
create_bd_cell -type ip -vlnv user.org:user:field_magnitude_bd_adapter:1.0 field_magnitude_bd_adapter_0
connect_bd_net $CLKP    [get_bd_pins field_magnitude_bd_adapter_0/clk]
connect_bd_net $PIX_RST [get_bd_pins field_magnitude_bd_adapter_0/rst]
connect_bd_net [get_bd_pins fdtd_solver_bd_adapter_0/solver_done] [get_bd_pins field_magnitude_bd_adapter_0/start]
# field_magnitude done -> ping-pong swap AND solver free-run restart
connect_bd_net [get_bd_pins field_magnitude_bd_adapter_0/done] [get_bd_pins fdtd_solver_bd_adapter_0/frame_done]

# ---------------------------------------------------------------------------
#  Field BRAMs (ey/ex/bz) — field_magnitude muxes solver vs scan onto port A/B
# ---------------------------------------------------------------------------
proc wire_field_bram {prefix bram clkp} {
    ...
    foreach sig {addra ena wea dina addrb enb web dinb} {
        connect_bd_net [get_bd_pins $sol/${prefix}_${sig}] [get_bd_pins $mag/solver_${prefix}_${sig}]
    }
    foreach sig {addra ena wea dina} {
        connect_bd_net [get_bd_pins $mag/${prefix}_${sig}] [get_bd_pins $bram/${sig}]
    }
    connect_bd_net [get_bd_pins $bram/douta] [get_bd_pins $mag/${prefix}_douta] [get_bd_pins $sol/${prefix}_douta]
    foreach sig {addrb enb web dinb} {
        connect_bd_net [get_bd_pins $mag/${prefix}_${sig}] [get_bd_pins $bram/${sig}]
    }
    set sol_doutb [get_bd_pins -quiet $sol/${prefix}_doutb]
    if {[llength $sol_doutb] > 0} { connect_bd_net [get_bd_pins $bram/doutb] $sol_doutb }
}
...
wire_field_bram ey ey_bram $CLKP
wire_field_bram ex ex_bram $CLKP
wire_field_bram bz bz_bram $CLKP
...
foreach sig {s_mag_addra s_mag_ena s_mag_wea s_mag_dina} {
    connect_bd_net [get_bd_pins field_magnitude_bd_adapter_0/${sig}] [get_bd_pins s_mag_pingpong_ctrl_0/${sig}]
}
connect_bd_net [get_bd_pins field_magnitude_bd_adapter_0/done] [get_bd_pins s_mag_pingpong_ctrl_0/mag_done]
...
set_property CONFIG.NUM_MI {4} [get_bd_cells axi_ic_0]
...
connect_bd_net $PS_ARSTN [get_bd_pins axi_ic_0/S00_ARESETN] \
    ...
    [get_bd_pins axi_ic_0/M03_ARESETN]
...
connect_bd_net [get_bd_pins mag_mode_concat/dout]   [get_bd_pins field_magnitude_bd_adapter_0/mag_mode]
...
connect_bd_net [get_bd_pins fdtd_solver_bd_adapter_0/solver_checksum] [get_bd_pins axi_gpio_status/gpio_io_i]
...
connect_bd_net [get_bd_pins fdtd_solver_bd_adapter_0/solver_done]    [get_bd_pins status_concat/In0]
connect_bd_net [get_bd_pins cordic_source_adapter_0/source_valid]    [get_bd_pins status_concat/In1]
connect_bd_net [get_bd_pins field_magnitude_bd_adapter_0/done]       [get_bd_pins status_concat/In2]
connect_bd_net [get_bd_pins field_magnitude_bd_adapter_0/busy]       [get_bd_pins status_concat/In3]
connect_bd_net [get_bd_pins fdtd_solver_bd_adapter_0/source_latched] [get_bd_pins status_concat/In4]
...
# ---- axi_gpio_src (M03) — extra coherent source positions + enables ----
#   CH1 out: {4'b0, src_en[3:0]@[27:24], src_addr2[11:0]@[23:12], src_addr1[11:0]@[11:0]}
#   CH2 out: {20'b0, src_addr3[11:0]@[11:0]}
#   (source 0 address stays on gpio_ctrl CH2[11:0]; src_en[0] enables it.)
create_bd_cell -type ip -vlnv xilinx.com:ip:axi_gpio:2.0 axi_gpio_src
set_property -dict [list CONFIG.C_GPIO_WIDTH {32} CONFIG.C_GPIO2_WIDTH {32} \
    CONFIG.C_ALL_OUTPUTS {1} CONFIG.C_ALL_OUTPUTS_2 {1} CONFIG.C_IS_DUAL {1}] [get_bd_cells axi_gpio_src]
connect_bd_net $FCLK     [get_bd_pins axi_gpio_src/s_axi_aclk]
connect_bd_net $PS_ARSTN [get_bd_pins axi_gpio_src/s_axi_aresetn]
connect_bd_intf_net [get_bd_intf_pins axi_ic_0/M03_AXI] [get_bd_intf_pins axi_gpio_src/S_AXI]
make_slice slice_src_addr1 11 0  32
make_slice slice_src_addr2 23 12 32
make_slice slice_src_en    27 24 32
connect_bd_net [get_bd_pins axi_gpio_src/gpio_io_o] \
    [get_bd_pins slice_src_addr1/Din] [get_bd_pins slice_src_addr2/Din] [get_bd_pins slice_src_en/Din]
make_slice slice_src_addr3 11 0 32
connect_bd_net [get_bd_pins axi_gpio_src/gpio2_io_o] [get_bd_pins slice_src_addr3/Din]
connect_bd_net [get_bd_pins slice_src_addr1/Dout] [get_bd_pins fdtd_solver_bd_adapter_0/source_addr1]
connect_bd_net [get_bd_pins slice_src_addr2/Dout] [get_bd_pins fdtd_solver_bd_adapter_0/source_addr2]
connect_bd_net [get_bd_pins slice_src_addr3/Dout] [get_bd_pins fdtd_solver_bd_adapter_0/source_addr3]
connect_bd_net [get_bd_pins slice_src_en/Dout]    [get_bd_pins fdtd_solver_bd_adapter_0/source_en]
