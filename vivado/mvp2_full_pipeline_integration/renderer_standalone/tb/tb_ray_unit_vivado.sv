...
module tb_ray_unit;
    ...
    initial begin
        ...
        if (!$value$plusargs("HEIGHTMAP=%s", heightmap_path))
            heightmap_path = "heightmap.hex";
        if (!$value$plusargs("VECTORS=%s", vectors_path))
            vectors_path = "ray_unit_vectors.hex";

        $display("Loading heightmap from %s", heightmap_path);
        $display("Loading vectors from %s", vectors_path);
        $readmemh(heightmap_path, heightmap);
